...
class Account(models.Model):
    ...
    client=models.ForeignKey(Client, on_delete=models.SET_NULL,null=True) #client= nested 
    creation_date=models.DateField(auto_now_add=True)
    def __str__(self):
        return f'client : {self.client}, balance= {self.balance}'
    class Meta:
        db_table='accounts' #pour changer nom de la table
...
class Transaction(models.Model):
    ...
    def clean(self):
        if self.transactionType == TransactionType.TRANSFER and self.transfer_to_account is None :
            raise ValueError('You have to specify the transfer_to_account')
        if self.transactionType == TransactionType.WITHDRAW and self.amount>self.account.balance :
            raise ValueError("You can't withdraw more than {self.account.balance}")
